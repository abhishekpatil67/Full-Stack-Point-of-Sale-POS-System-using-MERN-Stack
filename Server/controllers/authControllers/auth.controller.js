...
function getAppUrl() {
    ...
    return (process.env.FRONTEND_URL || `http://http://localhost:5173`)
}
...
export async function registerHandler(req, res) {
    ...
    if (!result.success) {
        ...
        return (res.status(400).json({ success: result.success, message: "invalid credentials..." }))
    }
    ...
    try {
        ...
        if (user) {
            ...
            return (res.status(403).json({ success: false, message: "the email is already registed please login..." }))
        }
        ...
        console.log("request received and token creted now sending the email");
        ...
        if (!ok.success) {
            console.log("erro while sending the email");
            return (res.status(500).json({ success: ok.success, message: ok.message }))
        }

        return (res.status(200).json({ success: ok.success, message: ok.message }))
...
export async function verifyEmailHandler(req, res) {
    ...
    try {
        ...
        return (res.status(500).json({ success: false, message: "internal server error occured try again after some time" }))
    }
...
export async function loginHandler(req, res) {
    ...
    if (!user) {
        return (res.status(403).json({ success: false, message: "user not found. Enter valid email." }))
    }
    ...
    try {
        ...
        return (res.status(200).json({
            ...
            message: "user logged in successfully", accessToken: accessToken
        }))
...
export async function forgotPasswordHandler(req, res) {
    ...
    try {
        ...
        return (res.status(201).json({ success: true, message: "if an account exists with this account we will sent a email..." }))
    }
    catch (error) {
        console.log(error)
        return (res.status(500).json({ success: false, message: "Internal server error. Try again later..." }))
...
export async function resetPasswordHandler(req, res) {
    ...
    try {
        ...
        console.log(error)
        return (res.status(404).json({ success: false, message: "something went wrong. Try again later x3" }))
